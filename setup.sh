...
# Menu de seleção
show_menu() {
    echo -e "\n${azul}Selecione o tipo de instalação MCP:${reset}"
    echo -e "1) Google Calendar MCP"
    echo -e "2) Evolution API MCP"
    echo -e "3) Sair"
    echo -e "\n${amarelo}Digite o número da opção desejada:${reset}"
    read -r choice
}
...
## Função para instalar Evolution API MCP
install_evolution_mcp() {
...
# Se a opção 1 foi selecionada, continua com a instalação do Google Calendar
if [ "$choice" = "1" ]; then
    main
fi 
